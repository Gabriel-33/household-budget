...
const CategoriasList = () => {
  ...
  const deleteCategoria = async (idCategoria) => {
    if (window.confirm('Tem certeza que deseja excluir esta categória? As transações relacionadas à ela, também serão excluidas')) {
      try {
        await categoriaService.deleteCategorias(idCategoria);

        await loadCategorias();
        
      } catch (error) {
        alert('Erro ao excluir categória');
        console.error(error);
      }
    }
  };
            ...
            <div className="categorias-grid">
              {filteredCategorias.map(categoria => (
                <div key={categoria.id} className="categoria-card">
                  ...
                  <div className="categoria-info">
                    <button onClick={() => deleteCategoria(categoria.id)} className='btn-secondary'>Excluir Categória</button>
                  </div>
                </div>
              ))}
            </div>
