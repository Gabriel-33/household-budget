...
const TransacoesList = () => {
  ...
  const deleteTransaction = async (idTransaction) => {
    if (window.confirm('Tem certeza que deseja excluir esta transação?')) {
      try {
        await transacaoService.deleteTransactions(idTransaction);
                ...
                <div key={transacao.id} className="transacao-card">
                  <div className="transacao-header">
                    <div className="transacao">
                      <span className={`${transacao.tipo === 1 ? 'receita' : 'despesa'}`}>
                        <h3>{transacao.tipo === 1 ? '+' : '-'} {formatters.formatCurrency(transacao.valor || 0)}</h3>
                      </span>
                    </div>
                    ...
                    <div className="transacao-detalhes">
                      <small className="transacao-data">
                        Data:{formatters.formatDate(transacao.data)}
                      </small>
                    </div>
                  </div>
                  ...
                  <div className="transacao-detalhes">
                    <small className="transacao-data">
                      <button className='btn-secondary' onClick={() => deleteTransaction(transacao.id)}>Excluir transação</button>
                    </small>
                  </div>
                </div>
              ))}
