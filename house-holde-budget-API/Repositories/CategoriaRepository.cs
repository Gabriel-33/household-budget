...
public class CategoriaRepository : ICategoriaRepository
{
    ...
    public async Task<Categoria?> GetCategoriaById(int id, bool onlyFindAsync = false)
    {
        Categoria? categoria = null;
        
        if (onlyFindAsync)
            categoria = await _dbContext.Categorias.FindAsync(id);
        else
            categoria = await _dbContext.Categorias
                .AsNoTracking()
                .Include(c => c.Transacoes)  // ← ADICIONE AQUI
