...
public static class TransacaoEndpoints
{
    ...
    public static RouteGroupBuilder MapTransacaoEndpoints(this RouteGroupBuilder builder)
    {
        ...
        builder.MapDelete("/deleteTransaction/{id:int}", DeleteTransacao)
                    .WithOpenApi(GastosSummaries.AdminDeleteTransactionSpecification)
    ...
    private static async Task<IResult> DeleteTransacao(HttpContext context,
        [FromRoute] int id,
        [FromServices] ITransacaoController controller)
    {
        int deletedId;
        ...
        try
        {
            deletedId = await controller.DeleteTransacao(id);
        }
        catch (ValidationException e)
        {
            return Results.BadRequest(e.Message);
