...
public class TransacaoController : ITransacaoController
{
    ...
    /// <summary>
    /// Deleta uma transação especifíca.
    /// </summary>
    public async Task<int> DeleteTransacao(int id)
    {
        Transacao transacao = await _transacaoRepository.GetTransacaoById(id);
            
        if (transacao == null)
        {
            ValidationException exception = new("Id da transação inválido");
            _logger.Error("Id de transação inválido");
            throw exception;
        }
                
        _logger.Information("Deletando transação de ID[{Id}]", id);
        
        await _transacaoRepository.DeleteTransacao(id);
        
        await _transacaoRepository.FlushChanges();

        _logger.Information("Transação de ID[{Id}] deletada com sucesso", id);
