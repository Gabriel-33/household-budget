...
public class CategoriaController : ICategoriaController
{
    ...
    {
        _logger.Information("Validando ID da categória para deleção: ID[{Id}]", id);
        
        Categoria categoria = await _categoriaRepository.GetCategoriaById(id);
        
        if (categoria == null)
        {
            ValidationException exception = new("ID da categória inválido");
            _logger.Error(exception, "ID da categória inválido");
            throw exception;
        }
        
        // Cascade: deletar todas as transações da categória
        if (categoria.Transacoes.Any())
        {
            _logger.Information("Deletando {Count} transações da categória ID[{Id}]", 
                categoria.Transacoes.Count, id);
            
            foreach (var transacao in categoria.Transacoes.ToList())
            {
